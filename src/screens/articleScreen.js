...
export default function ArticleScreen({ route }) {
    ...
    const { isLoading, error, data: LawResp } = useGetLawQuery(route?.params?.id, {
    ...
    return (
        ...
        >
            {isLoading ? <View>
                ...
                : <>
                    <Text style={styles.heading}>{LawResp?.data?.title}</Text>
                    <Text style={styles.subheading}>{LawResp?.data?.description}</Text>
                    <View style={{ ...styles.metaContainer, marginHorizontal: (layout.width / 10) / 2 }}>
                        {[
                            {
                                label: "Section",
                                value: LawResp?.data?.numberOfSections ?? 24
                            },
                            {
                                label: "Chapter",
                                value: LawResp?.data?.numberOfChapters ?? 96
                            },
                            {
                                label: "Tags",
                                value: LawResp?.data?.tags?.length
                            }
                        ].map((item) => <Pressable style={{ ...styles.meta }}
                            onPress={() => navigation.navigate("Chapters", {
                                id: route?.params?.id ?? "65a3daa6966714d85c2db0f2"
                            })}

                        >
                            <Text style={{ fontWeight: "600", color: "#B8B8B9" }}>
                                {item.label}
                            </Text>
